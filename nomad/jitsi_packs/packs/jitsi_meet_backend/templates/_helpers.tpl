...
[[- $tracingProtocol := or (env "CONFIG_tracing_protocol") "http" ]]
        ...
        # receiver is fabio-routed on the alloy job, so grpc only works with a
        # custom endpoint. Image tags without the tracing templates ignore
        # these vars, so this is safe to enable ahead of the images.
        ENABLE_TRACING = "1"
        TRACING_PROTOCOL = "[[ $tracingProtocol ]]"
        # jicofo/jvb (jicoco OtlpHttpSpanExporter) need the full /v1/traces URL
        TRACING_ENDPOINT = "[[ template "tracing-otlp-base" . ]][[ if eq $tracingProtocol "http" ]]/v1/traces[[ end ]]"
        # prosody appends /v1/traces itself
