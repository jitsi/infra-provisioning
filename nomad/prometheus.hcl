...
- name: core_service_alerts
  ...
  - alert: Jicofo_ICE_Restarts_High
    expr: >-
      ...
      sum by (shard) (jitsi_jicofo_participants_current) unless (sum by (shard) (jitsi_jicofo_participants_current) < 20))[2m:]) > 50
    for: 5m
    labels:
      service: jitsi
      severity: smoke
...
- name: core_extended_service_alerts
  ...
  - alert: Jicofo_Jibris_Available_None
    expr: max_over_time(count(jitsi_jicofo_jibri_instances_available{role!="standalone"} == 0)[1m:]) > 0
    for: 1m
    labels:
      service: jitsi
      severity: warn
    annotations:
      summary: jibris are not available to some jicofos in ${var.dc}
      description: >-
        There are have been no jibris available to {{ $value }} jicofos in ${var.dc} over the past
        minute. This means that no jibri instances were available to record or stream meetings.
        If this only impacted a single jicofo there may be a problem with the shard. If several
        jicofos are reporting this, then there may be a problem with our jibri capacity and the
        group should be scaled up.
      dashboard_url: ${var.grafana_url}
      alert_url: https://${var.prometheus_hostname}/alerts?search=jibris_available_none
  - alert: Jicofo_Jibris_Missing
    expr: max by (shard) (jitsi_jicofo_jibri_instances{role!="standalone"}) < 1 and on (shard) (count_over_time(jitsi_jicofo_jibri_instances{role!="standalone}[10m:1m]) >= 10)
    for: 5m
    labels:
      service: jitsi
      severity: severe
  ...
  - alert: Jicofo_SIP-Jigasi_Missing
    expr: max by (shard) (jitsi_jicofo_jigasi_sip_count{role!="standalone"}) < 1 and on (shard) (count_over_time(jitsi_jicofo_jigasi_sip_count{role!="standalone"}[10m:1m]) >= 10)
    for: 5m
    labels:
      service: jitsi
      severity: smoke
  ...
  - alert: Jicofo_Transcribers_Missing
    expr: max by (shard) (jitsi_jicofo_jigasi_transcriber_count{role!="standalone"}) < 1 and on (shard) (count_over_time(jitsi_jicofo_jigasi_transcriber_count{role!="standalone"}[10m:1m]) >= 10)
    for: 5m
    labels:
      service: jitsi
      severity: severe
