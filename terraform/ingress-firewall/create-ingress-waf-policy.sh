...
[ -z "$S3_STATE_KEY" ] && S3_STATE_KEY="$ENVIRONMENT/ingress-waf-policy/terraform.tfstate"
...
if [[ $RET -eq 0 ]]; then
  echo "Ingress WAF Policy created successfully"
else
  echo "Error creating Ingress WAF Policy: $RET"
fi
