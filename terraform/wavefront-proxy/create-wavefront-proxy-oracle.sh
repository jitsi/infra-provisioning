...
[ -z "$INSTANCE_CONFIG_NAME" ] && INSTANCE_CONFIG_NAME="WfProxyInstanceConfig"
...
RESOURCE_NAME_ROOT="$ENVIRONMENT-$ORACLE_REGION-wavefront-proxy"
...
if [[ "$TERRAFORM_MAJOR_VERSION" == "v1" ]]; then
  ...
  TF_GLOBALS_CHDIR_SG="-chdir=$LOCAL_PATH/wavefront-proxy-security-group"
  TF_POST_PARAMS=
  TF_POST_PARAMS_SG=
else
  TF_POST_PARAMS="$LOCAL_PATH"
  TF_POST_PARAMS_SG="$LOCAL_PATH/wavefront-proxy-security-group"
fi
...
if [ $? -eq 0 ]; then
  SECURITY_GROUP_ID="$(cat $LOCAL_SG_KEY | jq -r '.resources[]
...
if [ -z "$SECURITY_GROUP_ID" ]; then
  ...
  terraform $TF_GLOBALS_CHDIR_SG $ACTION \
