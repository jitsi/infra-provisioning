...
locals {
  hub_tags = {
    ...
    "${var.tag_namespace}.grid-role" = "hub"
...
resource "oci_core_instance_configuration" "oci_instance_configuration_hub" {
  lifecycle {
      create_before_destroy = true
  }

  compartment_id = var.compartment_ocid
  display_name = "${var.resource_name_root}-HubInstanceConfiguration"
...
resource "oci_core_instance_pool" "oci_instance_pool_hub" {
  compartment_id = var.compartment_ocid
  instance_configuration_id = oci_core_instance_configuration.oci_instance_configuration_hub.id  
  display_name = "${var.grid_name} Grid Hub"
  size = 1
  ...
  load_balancers {
    ...
    backend_set_name = var.load_balancer_bs_name
    port = 4444
...
resource "oci_core_instance_pool" "oci_instance_pool_node" {
  ...
  depends_on = [oci_core_instance_pool.oci_instance_pool_hub]
}

data "oci_core_instance_pool_instances" "oci_instance_pool_instances_hub" {
  compartment_id = var.compartment_ocid
  instance_pool_id = oci_core_instance_pool.oci_instance_pool_hub.id
  depends_on = [oci_core_instance_pool.oci_instance_pool_hub]
}
...
data "oci_core_instance" "oci_instance_datasources_hub" {
  depends_on = [oci_core_instance_pool.oci_instance_pool_hub]
  count = 1
  instance_id = lookup(data.oci_core_instance_pool_instances.oci_instance_pool_instances_hub.instances[count.index], "id")
}
...
locals {
  hub_private_ips = data.oci_core_instance.oci_instance_datasources_hub.*.private_ip
  node_private_ips = data.oci_core_instance.oci_instance_datasources_node.*.private_ip
}

resource "null_resource" "verify_cloud_init_hub" {
  count = 1
  depends_on = [data.oci_core_instance.oci_instance_datasources_hub]
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type = "ssh"
      host = element(local.hub_private_ips, count.index)
      user = var.user
