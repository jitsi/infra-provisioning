...
if [[ ${#AVAILABILITY_DOMAINS_SHAPE_A_1[@]} -gt 0 ]]; then
  ...
  AVAILABLE_SHARD_COUNT_A_1=$(($GET_AVAILABLE_RETURN_VALUE / $INSTANCE_POOL_SIZE_E_4))
else
...
if [[ $OUT_SHARD_COUNT_UNPROCESSED -ge 1 ]]; then
  ...
  echo "There is space only for $OUT_SHARD_COUNT_SHAPE_A_1 shards/pools of shape $SHAPE_A_1 and size $INSTANCE_POOL_SIZE_A_1, $OUT_SHARD_COUNT_SHAPE_E_4 shards/pools of shape $SHAPE_E_4 and size $INSTANCE_POOL_SIZE_E_4, $OUT_SHARD_COUNT_SHAPE_E_3 shards/pools of shape $SHAPE_E_3 and size $INSTANCE_POOL_SIZE_E_3, as well as $OUT_SHARD_COUNT_SHAPE_2_4 shards/pools of shape $SHAPE_2_4 and size $INSTANCE_POOL_SIZE_2_4, as well as $OUT_SHARD_COUNT_SHAPE_1_4 shards/pools of shape $SHAPE_1_4 and size $INSTANCE_POOL_SIZE_1_4"
  exit 2
fi
