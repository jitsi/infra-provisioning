...
CONSUL_HOST_AWS="consul-$AWS_CONSUL_ENV-$AWS_LOCAL_DATACENTER.$TOP_LEVEL_DNS_ZONE_NAME"
CONSUL_HOST_OCI="$OCI_LOCAL_DATACENTER-consul.$TOP_LEVEL_DNS_ZONE_NAME"

if [[ "$CONSUL_INCLUDE_AWS" == "true" ]]; then
    CONSUL_HOST="$CONSUL_HOST_AWS"
...
function loopDataCenters {
    ...
    for DC in $DC_LIST; do
        ...
        RESPONSE=$(curl --resolve $CONSUL_HOST_OCI:$PORT_OCI:127.0.0.1 --resolve $CONSUL_HOST_AWS:$PORT:127.0.0.1 $CURL_PARAM -X $METHOD $KV_URL)
        if [ $? -gt 0 ]; then
...
if [ -z "$DATACENTERS" ]; then
    ...
    if [[ "$CONSUL_INCLUDE_AWS" == "true" ]]; then
        ...
        AWS_DATACENTERS=$(curl --resolve $CONSUL_HOST_AWS:$PORT:127.0.0.1 -G $CONSUL_URL/v1/catalog/datacenters 2>/tmp/dclist)
        if [[ $? -gt 0 ]]; then
            AWS_DATACENTERS='[]'
        fi
    else
    ...
    if [[ "$CONSUL_INCLUDE_OCI" == "true" ]]; then
        ...
        OCI_DATACENTERS=$(curl --resolve $CONSUL_HOST_OCI:$PORT_OCI:127.0.0.1 -G $OCI_CONSUL_URL/v1/catalog/datacenters 2>>/tmp/dclist)
        if [[ $? -gt 0 ]]; then
            OCI_DATACENTERS='[]'
        fi
    else
