...
[ -z "$JIGASI_BUILD_ID" ] && JIGASI_BUILD_ID="$BUILD_ID"
[ -z "$JIGASI_BUILD_ID" ] && JIGASI_BUILD_ID="standalone"
...
#use the latest build of JIGASI by default
if [ -z "$JIGASI_VERSION" ]; then
  JIGASI_VERSION='*'
else
  [ "$JIGASI_VERSION" == "*" ] || echo "$JIGASI_VERSION" | grep -q -- -1$ || JIGASI_VERSION="${JIGASI_VERSION}-1"
fi
...
if [ ! -z "$EXISTING_IMAGE_OCID" ]; then
  if $FORCE_BUILD_IMAGE; then
    ...
    echo "Jigasi image already exists. Exiting..."
    exit 0
  fi
fi
...
#TODO delete oldest jigasi image if no space is available
