...
# TODO query available standard images with ubuntu 20.04
[ -z "$BARE_IMAGE_ID" ] && BARE_IMAGE_ID=$DEFAULT_JAMMY_IMAGE_ID

EXISTING_IMAGE_OCID=$(../all/bin/oracle_custom_images.py --type JammyBase --version "latest" --region="$ORACLE_REGION" --compartment_id="$COMPARTMENT_OCID" --tag_namespace="$TAG_NAMESPACE")
...
rm -f /tmp/fact.d/prod/default
rm -f /tmp/fact.d/stage/default
...
../all/bin/build-base-oracle.json
