...
if [[ "$IMAGE_ARCH" == "aarch64" ]]; then
  BARE_IMAGE_ID="$ARM_NOBLE_IMAGE_ID"
fi

# TODO query available standard images with ubuntu 22.04
[ -z "$BARE_IMAGE_ID" ] && BARE_IMAGE_ID=$DEFAULT_NOBLE_IMAGE_ID
[ -z "$BASE_IMAGE_TYPE" ] && BASE_IMAGE_TYPE="NobleBare"

EXISTING_IMAGE_OCID=$($LOCAL_PATH/oracle_custom_images.py --type NobleBase --version "latest" --architecture "$IMAGE_ARCH" --region="$ORACLE_REGION" --compartment_id="$COMPARTMENT_OCID" --tag_namespace="$TAG_NAMESPACE")
...
if [[ $RET -eq 0 ]]; then
  IMAGE_OCID=$($LOCAL_PATH/oracle_custom_images.py --type NobleBase --version "latest" --architecture "$IMAGE_ARCH" --region="$ORACLE_REGION" --compartment_id="$COMPARTMENT_OCID" --tag_namespace="$TAG_NAMESPACE")
