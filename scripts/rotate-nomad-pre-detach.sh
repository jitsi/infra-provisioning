...
echo "## rotate-nomad-pre-detch: getting private IP"
...
echo "## rotate-nomad-pre-detach drain nomad node on $INSTANCE_PRIMARY_PRIVATE_IP with user $SSH_USER"
ssh -F $LOCAL_PATH/../config/ssh-vpn.config $SSH_USER@$INSTANCE_PRIMARY_PRIVATE_IP 'nomad node eligibility -self -disable; nomad node drain -self -enable -force -detach -yes'
RET=$?
if [[ $RET -gt 0 ]]; then
    echo "## ERROR draining nomad on $INSTANCE_PRIMARY_PRIVATE_IP with code $RET"
fi

echo "## rotate-nomad-pre-detach waiting while workloads are removed"
sleep 90

echo "## rotate-nomad-pre-detach shutting down nomad service on $INSTANCE_PRIMARY_PRIVATE_IP with user $SSH_USER"
...
echo "## rotate-nomad-pre-detach graceful consul leave on $INSTANCE_PRIMARY_PRIVATE_IP with user $SSH_USER"
...
echo "## rotate-nomad-pre-detach shutting down consul service on $INSTANCE_PRIMARY_PRIVATE_IP with user $SSH_USER"
