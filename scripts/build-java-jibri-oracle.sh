...
EXISTING_IMAGE_OCID=$($LOCAL_PATH/oracle_custom_images.py --type JavaJibri --version "$JIBRI_VERSION" --architecture "$IMAGE_ARCH" --region="$ORACLE_REGION" --compartment_id="$COMPARTMENT_OCID" --tag_namespace="$TAG_NAMESPACE")
...
[ -z "$BASE_IMAGE_TYPE" ] && BASE_IMAGE_TYPE="$JIBRI_BASE_IMAGE_TYPE"
...
for CLEAN_ORACLE_REGION in $ORACLE_IMAGE_REGIONS; do
  ...
  $LOCAL_PATH/oracle_custom_images.py --clean $ORACLE_CUSTOM_IMAGE_LIMIT --architecture "$IMAGE_ARCH" --delete --region=$CLEAN_ORACLE_REGION --type=JavaJibri --compartment_id=$TENANCY_OCID;
done
