...
if [ "$getGroupHttpCode" == 404 ]; then
  ...
  findGroup
  if [ "$getGroupHttpCode" == 404 ]; then
    echo "No group $GROUP_NAME found at $AUTOSCALER_URL. Skipping autoscaler cleanup"
    GROUP_NOT_FOUND=true
  elif [ "$getGroupHttpCode" == 000 ]; then
    echo "Local autoscaler not present for $GROUP_NAME. Skipping autoscaler cleanup"
    GROUP_NOT_FOUND=true
...
if [[ "$CLOUD_PROVIDER" == "nomad" ]] || [[ "$SHARD_CORE_CLOUD_PROVIDER" == "nomad" ]]; then
  ...
  sleep 30
  $LOCAL_PATH/nomad.sh job stop -purge jvb-$SHARD
fi
