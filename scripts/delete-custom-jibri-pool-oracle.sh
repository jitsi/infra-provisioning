...
# Scale down custom group to 0
##################################

export GROUP_NAME
export ORACLE_REGION
export TYPE="java-jibri"
export TOKEN
...
if [ "$RESULT" -eq 0 ]; then
  ...
  if [ "$GROUP_REPORT_STATUS_CODE" == 200 ]; then
    ...
    for INSTANCE_ID in $INSTANCES; do
      echo "Terminating Jibri instance $INSTANCE_ID"
...
##################################
INSTANCE_CONFIGURATIONS=$(oci compute-management instance-configuration list --region "$ORACLE_REGION" -c "$COMPARTMENT_OCID" --sort-by TIMECREATED --sort-order DESC --all --query 'data[?"defined-tags".'\"$TAG_NAMESPACE\"'."shard-role" == `'"java-jibri"'`]' | jq -r .[].id)
