...
function list() {
    ...
    if [[ "$SHARDS_FROM_CONSUL" == "true" ]]; then
        if [ ! -z "$ORACLE_REGION" ]; then
            OCI_DC="$ENVIRONMENT-$ORACLE_REGION"
            DATACENTERS="[\"$OCI_DC\"]" OCI_DATACENTERS="$OCI_DC" CONSUL_INCLUDE_AWS="$CONSUL_INCLUDE_AWS" CONSUL_INCLUDE_OCI="$CONSUL_INCLUDE_OCI" RELEASE_INVERSE="$RELEASE_INVERSE" RELEASE_NUMBER="$RELEASE_NUMBER" ENVIRONMENT="$ENVIRONMENT" DISPLAY="shards" SERVICE="signal" $LOCAL_PATH/consul-search.sh $ANSIBLE_SSH_USER
        else
        ...
        [ ! -z "$ORACLE_REGION" ] && REGION_PARAM="--region $ORACLE_REGION"
        $LOCAL_PATH/shard.py --environment=$ENVIRONMENT --list $RELEASE_PARAM $REGION_PARAM
    fi
}
