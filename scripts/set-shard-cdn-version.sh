...
wget -q https://web-cdn.jitsi.net/$CDN_PREFIX$CDN_VERSION/libs/lib-jitsi-meet.min.js.map
...
for i in `cat $SIGNAL_INVENTORY_PATH`; do
    ...
    scp -F "$LOCAL_PATH/../config/ssh.config" $LIST_FILES_FROM_ROOT_DIR ./external_api.min.js.map ./external_api.min.js $ANSIBLE_SSH_USER@$i:
done
