...
if [ -z $BASE_IMAGE_ID ]; then
  echo "No BASE_IMAGE_ID provided or found, and no DEFAULT_FOCAL_EC2_IMAGE_ID defined for region $EC2_REGION.  Exiting..."
fi
...
ansible-playbook $ANSIBLE_BUILD_PATH/build-jvb.yml -i "somehost," \
-e "build_id=$BUILD_ID" \
-e "jitsi_videobridge_deb_pkg_version=$JVB_VERSION" \
-e "jitsi_meet_meta_version=$JITSI_MEET_META_VERSION" \
-e "ansible_ssh_user=$ANSIBLE_SSH_USER" \
-e "ec2_keypair=$EC2_SSH_KEYPAIR" \
-e "ec2_image_id=$BASE_IMAGE_ID" \
