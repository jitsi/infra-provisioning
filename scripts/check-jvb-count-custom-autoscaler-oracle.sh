...
while true; do
  ...
  if [ "$GROUP_REPORT_STATUS_CODE" == 200 ]; then
    ...
    SHUTDOWN_COUNT=$(echo "$GROUP_REPORT_VALUE" | jq -r '.groupReport.instances|map(select(.cloudStatus=="SHUTDOWN" or .cloudStatus=="unknown" or .shutdownComplete!=false))|length')
