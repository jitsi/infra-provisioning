...
if region in ORACLE_REGION_MAP:
  ...
  if len(shard_instances) == 0:
    ...
    update_instance_tags(shard_instances[0], new_freeform_tags={SHARD_STATE_TAG:STATE})
    print('OCI Tags applied in '+region+' for '+ENVIRONMENT_TAG+': '+ENVIRONMENT+', '+SHARD_TAG+': '+SHARD+', '+SHARD_STATE_TAG+': '+STATE)
else:
  # AWS
  ...
  if len(instance_ids) == 0:
    ...
    ec2.create_tags(DryRun=False, Resources=instance_ids, Tags=[{'Key':SHARD_STATE_TAG, 'Value':STATE}])
    print('AWS Tags applied in '+region+' for '+ENVIRONMENT_TAG+': '+ENVIRONMENT+', '+SHARD_TAG+': '+SHARD+', '+SHARD_STATE_TAG+': '+STATE)